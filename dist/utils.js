 ...
 */
export function authorisedFetch(url, authToken, otherOptions, requiresJSONContentHeader = false) {
 ...
 */
export function buildErrorMessage(message, res) {
 ...
 */
export function hideApiKey(key) {
    const keyWithoutPrefix = key.replace(/^tskey-api-/, "");
    if (keyWithoutPrefix.length < 2)
        return "####";
